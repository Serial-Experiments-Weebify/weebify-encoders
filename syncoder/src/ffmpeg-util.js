...
function $(strings, ...values) {
    ...
    return new Promise((resolve, reject) =>
        childProc.exec(command, (error, stdout) => {
...
async function ffprobe(file) {
    return JSON.parse(
        await $`ffprobe -v quiet -print_format json -show_format -show_streams "${file}"`
    );
}

async function getDuration(file) {
    return parseFloat(
        await $`ffprobe -i "${file}" -show_entries format=duration -v quiet -of "csv=p=0"`
    );
}

function ffmpegGetStream(file, index, format) {
    return childProc.spawn(
        "ffmpeg",
        ["-i", file, "-map", `0:${index}`, "-f", format, "-"],
        {}
    );
}
...
async function ffmpegEncode(file, streamData, duration, hres, outFile) {
    ...
    if (streamData.subtitles)
        filters.push(
            `subtitles=${ffEscape(ffEscape(file))}:si=${
                streamData.subtitles.subIndex
            }`
        );
